...
class MediaDevice:
    ...
    def _promptMediaSources(self):
        mediaSources = questionary.text(
            "Enter Media Source names (comma separated)"
        ).ask()
        if "," in mediaSources:
    ...
    def _learnCommand(self, key: str, nestedKey: str):
        if key and nestedKey:
            print(
                f"Learning {key.upper()} {nestedKey.upper()} - Point the remote to the device and press the button"
            )
        elif key:
            print(
                f"Learning {key.upper()} - Point the remote to the device and press the button"
            )

        command = async_learn(self.device)

        choice = input(
            f"Press Enter or Y to confirm or N to re-learn Command - {command}\n"
        )
